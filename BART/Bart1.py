...
class BartApi(object):
    ...
    def get_estimated_times(self, origin_station_name='fremont', line_final_station_name=None):
        ...
        origin_station_name = origin_station_name.lower()
        ...
        line_final_station_name = line_final_station_name.lower()
    ...
    def get_station_name_dict(self):
        ...
        payload_dict = json.loads(response.text)
        # json output returns a dictionary - which comprises of a list of stations
        # which is further comprised of dict
...
if __name__ == "__main__":
    ...
    print('\nETD from Downtown Berkeley')
    etd_dict = bart_api.get_estimated_times(origin_station_name='downtown berkeley')
    for key, value in etd_dict.items():
        print("%20s: %s" % (key, value))

    print('\nETD from Fremont to Richmond')
    etd_dict = bart_api.get_estimated_times(origin_station_name='fremont',
                                            line_final_station_name='richmond')
    for key, value in etd_dict.items():
        print("%20s: %s" % (key, value))
